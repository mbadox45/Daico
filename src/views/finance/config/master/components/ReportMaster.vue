<script setup>
    ...
    const formDatabase = (cond, data) => {
        ...
        title_dialog.value = cond == 'add' ? 'Managemnt Report - Tambah Data' : cond == 'edit' ? 'Managemnt Report - Edit Data' : 'Managemnt Report - Hapus Data' ;
        if (cond == 'add') {
            resetForm()
        } else {
            forms.value = {
    ...
    const saveData = async () => {
        ...
        if (forms.value.nama != null) {
            if (status_form.value == 'add') {
                const response = await ReportMaster.addReport(forms.value);
                const load = response.data;
                ...
                const response = await ReportMaster.updateReport(forms.value.id, forms.value);
                const load = response.data;
                if (load.success == true) {
                    messages.value = [
    ...
    <div class="flex-auto flex flex-column gap-3 p-3 bg-white shadow-3">
        ...
        <Dialog v-model:visible="visible" modal :header="title_dialog" :style="{ width: '50rem' }">
            ...
            <div class="flex flex-column gap-3">
                <label for="username" class="font-semibold">Management Report <small class="text-red-500">*</small></label>
            ...
            <template #list="slotProps">
                <div class="col-12">
                    <div class="flex flex-column md:flex-row align-items-center p-3 w-full border-1 my-2 border-round border-gray-300">
                        <img :src="'/images/report.png'" :alt="slotProps.data.nama" class="my-4 md:my-0 w-6 md:w-4rem mr-5" />
