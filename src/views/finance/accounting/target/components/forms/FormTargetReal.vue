<script setup>
    ...
    import { addReal_TargetController, updateReal_TargetController } from '@/controller/retail/TargetController.js'
    ...
    const postData = async() => {
        try {
            if (forms.value.tanggal != null && forms.value.value != null && forms.value.product_type != null && forms.value.productable_id != null) {
                if (status == 'add') {
                    const response = await addReal_TargetController(forms.value);
                    ...
                    const response = await updateReal_TargetController(forms.value.id,forms.value);
                    if (response.status == true) {
                        emit('submit','sukses');
                    } else {
                        emit('submit','error');
