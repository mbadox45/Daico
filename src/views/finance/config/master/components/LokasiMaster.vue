...
// API ========================================================================================================================================================
import {
  loadAll_LocationMaster,
  add_LocationMaster,
  update_LocationMaster,
} from "@/controller/master_data/LocationController.js";
...
const saveData = async () => {
  ...
  if (forms.value.name != null) {
    ...
    if (status_form.value == "add") {
      const response = await add_LocationMaster(forms.value);
      ...
      const response = await update_LocationMaster(forms.value.id, forms.value);
      if (response.status == true) {
        messages.value = [
          { severity: "success", content: response.msg, id: count.value++ },
        ];
...
<template>
  <div class="flex-auto flex flex-column gap-3 p-3 bg-white shadow-3">
    <span class="font-medium text-xl">Data Location</span>
    ...
    >
      <template #empty> No location found. </template>
      ...
      <Column field="nama" style="min-width: 8rem">
        <template #body="{ data }">
          ...
          >
            <i class="pi pi-map-marker text-3xl my-4 md:my-0 mr-5"></i>
            <div class="flex-1 text-center md:text-left">
              <div class="font-bold text-2xl">{{ data.name }}</div>
              <div class="flex align-items-center">
                <!-- <i class="pi pi-shield mr-2 text-green-300"></i> -->
                <span class="font-normal text-gray-600">Location</span>
              </div>
            </div>
            <div
