    ...
    const saveData = async () => {
        ...
        if (forms.value.nama != null) {
            if (status_form.value == 'add') {
                const response = await CentreMaster.addCentre(forms.value);
                const load = response.data;
                ...
                const response = await CentreMaster.updateCentre(forms.value.id, forms.value);
                const load = response.data;
                if (load.success == true) {
                    messages.value = [
...
<template>
    <div class="flex-auto flex flex-column gap-3 p-3 bg-white shadow-3">
        <span class="font-medium text-xl">Cost Centre</span>
        ...
        <Dialog v-model:visible="visible" modal :header="title_dialog" :style="{ width: '50rem' }">
            ...
            <div class="flex flex-column gap-3">
                <label for="username" class="font-semibold">Cost Centre <small class="text-red-500">*</small></label>
            ...
            <template #list="slotProps">
                <div class="col-12">
                    <div class="flex flex-column md:flex-row align-items-center p-3 w-full border-1 my-2 border-round border-gray-300">
                        <img :src="'/images/arrows.png'" :alt="slotProps.data.nama" class="my-4 md:my-0 w-6 md:w-4rem mr-5" />
                        <div class="flex-1 text-center md:text-left">
                            <div class="font-bold text-2xl">{{ slotProps.data.nama }}</div>
                            <div class="flex align-items-center">
                                <i class="pi pi-building mr-2 text-green-300"></i>
                                <span class="font-normal text-gray-600">Cost Centre</span>
                            </div>
                        </div>
