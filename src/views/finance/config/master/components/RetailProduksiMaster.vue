...
// API ========================================================================================================================================================
import {
  loadAll_RetailProdMaster,
  add_RetailProdMaster,
  update_RetailProdMaster,
} from "@/controller/master_data/RetailProduksiController.js";
...
const loadData = async () => {
  try {
    ...
    const data = await loadAll_RetailProdMaster();
    const list = [];
    if (data != null) {
...
const formDatabase = (cond, data) => {
  ...
  title_dialog.value = cond == "add" ? "Retail Produksi - Tambah Data" : cond == "edit" ? "Retail Produksi - Edit Data" : "Retail Produksi - Hapus Data";
  if (cond == "add") {
    resetForm();
  } else {
    forms.value = {
...
const saveData = async () => {
  ...
  if (forms.value.name != null && forms.value.name != "") {
    ...
    if (status_form.value == "add") {
      const response = await add_RetailProdMaster(forms.value);
      ...
      const response = await update_RetailProdMaster(forms.value.id, forms.value);
      if (response.status == true) {
        messages.value = [
          { severity: "success", content: response.msg, id: count.value++ },
        ];
...
<template>
  <div class="flex-auto flex flex-column gap-3 p-3 bg-white shadow-3">
    ...
    >
      <template #empty> No retail produksi found. </template>
      <template #loading> Loading retail produksi data. Please wait. </template>
